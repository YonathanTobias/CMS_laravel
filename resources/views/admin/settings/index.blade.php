        ...
        <div class="p-4 bg-blue-50/50 dark:bg-blue-950/20 rounded-2xl border border-blue-100 dark:border-blue-900/50 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                    <label class="block text-[11px] font-bold uppercase text-blue-900 dark:text-sky-300 mb-1">
                        <i class="fa-solid fa-expand mr-1"></i> Mode Penyesuaian Ukuran Gambar (Fit & Stretch)
                    </label>
                    <select name="profile_image_fit" class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-blue-500">
                        <option value="cover" {{ ($settings['profile_image_fit'] ?? 'cover') === 'cover' ? 'selected' : '' }}>
                            🖼️ Cover / Crop Penuh (Mengisi Seluruh Bingkai Penuh)
                        </option>
                        <option value="contain" {{ ($settings['profile_image_fit'] ?? '') === 'contain' ? 'selected' : '' }}>
                            🔍 Fit Utuh / Pas (Contain - Gambar Kebesaran Dipaskan Utuh Tanpa Terpotong)
                        </option>
                        <option value="fill" {{ ($settings['profile_image_fit'] ?? '') === 'fill' ? 'selected' : '' }}>
                            ↔️ Stretch / Melar (Fill - Ditarik Mengikuti Seluruh Ukuran Kotak)
                        </option>
                    </select>
                </div>
            </div>
