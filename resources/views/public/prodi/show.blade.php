    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div class="lg:col-span-8 space-y-10">
                <!-- Deskripsi Umum -->
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
                    <h2 class="font-heading font-bold text-2xl text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3">Profil & Gambaran Umum</h2>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
                    <h2 class="font-heading font-bold text-2xl text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3 flex items-center gap-2">
                        <i class="fa-solid fa-book-open text-blue-700 dark:text-sky-400"></i> Kurikulum & Beban Studi
                    </h2>
                    <p class="text-slate-700 dark:text-slate-300 leading-relaxed">{!! nl2br(e($prodi->curriculum_summary)) !!}</p>
                </div>
                @endif
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
                    <h2 class="font-heading font-bold text-2xl text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3 flex items-center gap-2">
                        <i class="fa-solid fa-briefcase text-blue-700 dark:text-sky-400"></i> Prospek Kerja & Lapangan Karir
                    </h2>
                    <p class="text-slate-700 dark:text-slate-300 leading-relaxed">{!! nl2br(e($prodi->career_prospects)) !!}</p>
                </div>
                @endif
            </div>
            ...
            <div class="lg:col-span-4 space-y-8">
                <div class="bg-gradient-to-br from-blue-900 to-blue-950 text-white rounded-2xl p-8 shadow-xl space-y-6 border border-blue-800">
                    <h3 class="font-heading font-bold text-2xl">Daftar PMB Online</h3>
                    <p class="text-xs text-blue-200 leading-relaxed">Siapkan diri Anda menjadi bagian dari civitas akademika STIKes Panti Waluya Malang.</p>
                    <a href="{{ \App\Models\SiteSetting::get('pmb_link', '#') }}" target="_blank" class="block text-center bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-extrabold py-3.5 rounded-xl shadow-lg transition text-xs">
                        Pendaftaran Mahasiswa Baru &rarr;
                    </a>
                </div>

                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
                    <h3 class="font-heading font-bold text-lg text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3">Program Studi Lainnya</h3>
                    @php
                        $otherProdis = \App\Models\ProgramStudi::where('id', '!=', $prodi->id)->get();
                    @endphp
                    <div class="space-y-3 text-sm">
                        @foreach($otherProdis as $other)
                            <a href="{{ route('prodi.show', $other->slug) }}" class="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition text-slate-800 dark:text-slate-200 font-medium border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                                <span>{{ $other->name }}</span>
                                <span class="text-xs text-blue-600 dark:text-sky-400 font-bold">&rarr;</span>
                            </a>
                        @endforeach
                    </div>
                </div>
