                ...
                @if(!$hasTextContent)
                    @if(!empty(trim($slide->cta_link ?? '')))
                        <a href="{{ $slide->cta_link }}" class="relative block w-full h-full overflow-hidden cursor-pointer bg-slate-950" title="Klik untuk membuka tautan banner">
                            ...
                            <img src="{{ $heroImgSrc }}" class="absolute inset-0 w-full h-full object-cover blur-2xl opacity-40 scale-110 pointer-events-none">
                            <!-- Crisp Full Banner: 100% Fit & Utuh Tanpa Terpotong -->
                            <img src="{{ $heroImgSrc }}" alt="{{ $slide->title ?? 'Banner STIKes Panti Waluya' }}" class="relative z-10 w-full h-full object-contain">
                        </a>
                    @else
                        <div class="relative w-full h-full overflow-hidden bg-slate-950 flex items-center justify-center cursor-pointer" @click="openLightbox(@js($heroImgSrc), @js($slide->title))">
                            <!-- Ambient Glow Backdrop -->
                            <img src="{{ $heroImgSrc }}" class="absolute inset-0 w-full h-full object-cover blur-2xl opacity-40 scale-110 pointer-events-none">
                            <!-- Crisp Full Banner: 100% Fit & Utuh Tanpa Terpotong -->
                            <img src="{{ $heroImgSrc }}" alt="{{ $slide->title ?? 'Banner STIKes Panti Waluya' }}" class="relative z-10 w-full h-full object-contain">
                        ...
                        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                            <div class="max-w-2xl space-y-5">
                                ...
                                <div class="flex flex-wrap gap-3 pt-2 pointer-events-auto">
                                    ...
                                    @if(!empty(trim($slide->secondary_text ?? '')))
...
<!-- 5. Sekilas Mengenai Kami & Profil Kampus -->
<section class="py-16 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 transition-colors duration-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <!-- Right Column: Profil Singkat, Visi Misi & Sambutan -->
            <div class="lg:col-span-7 space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-bold text-slate-800 dark:text-slate-200">
                    <div class="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-sky-400 flex items-center justify-center text-lg shrink-0">
                    ...
                    <div class="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 flex items-center justify-center text-lg shrink-0">
                ...
                <div class="pt-2">
                    <a href="{{ route('pages.show', 'profil') }}" class="inline-flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white font-bold px-6 py-3 rounded-xl shadow transition text-xs">
                        Lihat Selengkapnya Profil Kampus &rarr;
                    </a>
                </div>
            </div>
...
<section class="py-16 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 transition-colors duration-200"
         x-data="{ 
            modalCert: null,
            scrollNextCert() {
                this.$refs.certCarousel.scrollBy({ left: 340, behavior: 'smooth' });
            },
            scrollPrevCert() {
                this.$refs.certCarousel.scrollBy({ left: -340, behavior: 'smooth' });
            }
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 p-8 shadow-sm hover:shadow-xl transition duration-300 flex flex-col justify-between group">
                    <div>
                        ...
                        @if($prodi->accreditation_certificate || $prodi->rpl_certificate)
                            ...
                            @endphp
                            <div class="mt-4 mb-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3" x-data="{ certTab: '{{ $prodi->accreditation_certificate ? 'accreditation' : 'rpl' }}' }">
